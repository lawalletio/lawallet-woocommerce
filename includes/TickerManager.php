...
class TickerManager {
  ...
  public function setExchange($exchangeSlug) {
    ...
    if (!$exchange->hasFiat($this->fiat)) {
      ...
      throw new \Exception( __('Can\'t load Exchange.', 'lawallet-woocommerce' ), 2);
    }
  ...
  public function unmatchedTicker($exchange) {
    ...
    <div class="error notice">
      <p><b></n><?=__( 'LND Woocommerce disabled', 'lawallet-woocommerce' ) ?></b></p>
      <div>
        <p>
          <? printf( __( 'The current Exchange "%s" doesn\'t have ticker for %s currency.', 'lawallet-woocommerce' ), $exchange->name, $this->fiat ) ?>
        ...
        <p>
          <?=__( 'You should select another exchange or change the FIAT currency from Woocommerce.', 'lawallet-woocommerce' ) ?>
